...
- 日期精确到分钟，避免同一天多次扫描冲突
- 平台名用中文：起点、番茄、七猫、晋江、刺猬猫
- 跨平台扫描用：`{YYYYMMDD_HHmm}_all`
- 示例：`20260503_1430_起点/`、`20260503_1430_番茄/`、`20260503_1430_all/`
...
使用 `/browser-cdp` 启动 Chrome/Edge，直接抓取平台榜单页面的结构化数据。
...
输出前逐项检查：
- [ ] 扫榜报告包含市场概况 + 题材热度排行表 [必填]
- [ ] 题材热度排行表含排名/题材/榜上数量/趋势/代表作五列 [必填]
- [ ] 关键数据洞察含字数区间 + 更新频率 + 书名特征 + 标签热词 [必填]
- [ ] 值得关注的方向至少 3 条，每条含可行性评估 [必填]
- [ ] 一句话总结存在且犀利有观点 [必填]
- [ ] 原始数据文件已保存到 scan-data/{YYYYMMDD_HHmm}_{平台}/ [必填]
- [ ] 文件命名遵循规范：{平台}{榜单名称}_{YYYYMMDD}.md [必填]
