...
if [ ! -d "$CHROME_DEBUG_PROFILE/Default" ]; then
    ...
    cp -a "$CHROME_DEFAULT_PROFILE/Default" "$CHROME_DEBUG_PROFILE/Default"
    echo "✅ Profile 已复制到: $CHROME_DEBUG_PROFILE"
else
    echo "✅ 调试用 profile 已存在于: $CHROME_DEBUG_PROFILE"
    # 可选：刷新 Cookie 和登录数据
    echo "📋 正在刷新 Cookie 和登录数据..."
    cp -pf "$CHROME_DEFAULT_PROFILE/Default/Cookies" "$CHROME_DEBUG_PROFILE/Default/Cookies" 2>/dev/null || true
    cp -pf "$CHROME_DEFAULT_PROFILE/Default/Login Data" "$CHROME_DEBUG_PROFILE/Default/Login Data" 2>/dev/null || true
fi
...
# 确认所有 Chrome 进程已退出
REMAINING=$(pgrep -c "Google Chrome" 2>/dev/null || echo 0)
