...
所有短篇写作文件统一存放到项目根目录下的 `novels/` 目录。
...
1. 所有文件在 `novels/{标题}/` 目录下创建
2. 对标参考的拆文报告从 `analyze-data/{参考小说}/` 获取（由 story-short-analyze 输出）
3. 创建目录：
   - macOS：`mkdir -p novels/{标题}/`
   - Windows：`New-Item -ItemType Directory -Force -Path novels/{标题}/`
...
3. 禁止「他说/她道」（用动作代替）
4. 禁止缩进
5. 禁止正文中使用 Markdown 格式渲染（加粗 `**`、斜体 `*`）。小节编号 `###1.` 和纯数字 `1.` 是结构性分隔符，属于排版标记，不属于此禁令。
...
框架确定后，完成设计任务，然后在 `novels/{标题}/` 目录下创建文件。
...
├── 正文.md          # 完整正文
└── 对标/            # 可选：story-short-analyze 输出（来自 analyze-data/{参考小说}/）
    └── {参考小说}/拆文报告.md
```
...
1 主事件 | 子事件A->子事件B->子事件C [对话|冲突|伏笔] | 情绪 | 读者新获知什么 | 钩子 | 伏笔/物件 | 动/静 | 对话密度
2 主事件 | ...
...
```
...
输出前逐项检查：
- [ ] 设定.md 包含基本信息、一句话梗概、核心反转、情绪设计、人设速写五个部分 [必填]
- [ ] 小节大纲.md 每节包含：主事件 | 子事件×3-5 | 情绪 | 读者新获知什么 | 钩子 | 伏笔/物件 | 动静 | 对话密度 [必填]
- [ ] 正文.md 段落不超过 60 字 [必填]
- [ ] 正文.md 段间无空行 [必填]
- [ ] 正文.md 对话独立成行、无「他说/她道」 [必填]
- [ ] 正文.md 小节编号格式统一（`###1.` 或纯数字 `1.`） [必填]
- [ ] 正文.md 全文 8-15 个小节，每节 800-1500 字 [必填]
- [ ] 正文.md 前 3 句包含开篇钩子，非环境描写 [必填]
- [ ] 结构物件三现追踪表已填入设定.md [可选]
- [ ] 反派设计已填入设定.md（如有反派） [可选]
